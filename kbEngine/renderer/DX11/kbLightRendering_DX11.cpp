...
void kbRenderer_DX11::RenderLight( const kbRenderLight *const pLight ) {
	...
	ID3D11ShaderResourceView *const  RenderTargetViews[] = {	GetRenderTarget(COLOR_BUFFER)->m_pShaderResourceView,
																GetRenderTarget(NORMAL_BUFFER)->m_pShaderResourceView,
																GetRenderTarget(SPECULAR_BUFFER)->m_pShaderResourceView,
																GetRenderTarget(DEPTH_BUFFER)->m_pShaderResourceView,
																GetRenderTarget(SHADOW_BUFFER)->m_pShaderResourceView };
...
void kbRenderer_DX11::RenderShadow( const kbRenderLight *const pLight, kbMat4 splitMatrices[] ) {
	...
	m_pDeviceContext->ClearRenderTargetView( GetRenderTarget(SHADOW_BUFFER)->m_pRenderTargetView, clearColor );
	m_pDeviceContext->ClearDepthStencilView( GetRenderTarget(SHADOW_BUFFER_DEPTH)->m_pDepthStencilView , D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1, 0 );

	if ( m_bRenderToHMD ) {
		const float color[] = { 1.0f, 1.0f, 1.0f, 1.0f };
		m_pDeviceContext->ClearRenderTargetView( GetRenderTarget(SHADOW_BUFFER)->m_pRenderTargetView, color );
		return;
	}

	const float shadowBufferSize = (float) GetRenderTarget(SHADOW_BUFFER)->GetWidth();
	...
	// Render Shadow map
	m_pDeviceContext->OMSetRenderTargets( 1, &GetRenderTarget(SHADOW_BUFFER)->m_pRenderTargetView, GetRenderTarget(SHADOW_BUFFER_DEPTH)->m_pDepthStencilView );
	...
	for ( int i = 0; i < 4 && pLight->m_CascadedShadowSplits[i] < FLT_MAX; i++ ) {
		...
		std::map< const kbComponent *, kbRenderObject * >::const_iterator iter;
		for ( iter = m_pCurrentRenderWindow->GetRenderObjectMap().begin(); iter != m_pCurrentRenderWindow->GetRenderObjectMap().end(); iter++ ) {
...
void kbRenderer_DX11::RenderLightShafts() {
	...
	for ( int i = 0; i < m_LightShafts_RenderThread.size(); i++ ) {
		...
		// Generate Flare Mask
		{
			...
			m_pDeviceContext->OMSetRenderTargets( 1, &GetRenderTarget(SCRATCH_BUFFER)->m_pRenderTargetView, nullptr );
		
			D3D11_VIEWPORT viewport;
			viewport.TopLeftX = 0.0f;
			viewport.TopLeftY = 0.0f;
			viewport.Width = ( float )GetRenderTarget(SCRATCH_BUFFER)->GetWidth();
			viewport.Height = ( float )GetRenderTarget(SCRATCH_BUFFER)->GetHeight();
			viewport.MinDepth = 0;
			viewport.MaxDepth = 1.0f;
			m_pDeviceContext->RSSetViewports( 1, &viewport );

			ID3D11ShaderResourceView *const  RenderTargetViews[] = { (ID3D11ShaderResourceView*)CurLightShafts.m_pTexture->GetGPUTexture(), GetRenderTarget(DEPTH_BUFFER)->m_pShaderResourceView };
		...
		// Flare out here
		{
			...
			m_pDeviceContext->ClearRenderTargetView( GetRenderTarget(DOWN_RES_BUFFER)->m_pRenderTargetView, color );
			...
			ID3D11ShaderResourceView *const  RenderTargetViews[] = { GetRenderTarget(SCRATCH_BUFFER)->m_pShaderResourceView };
