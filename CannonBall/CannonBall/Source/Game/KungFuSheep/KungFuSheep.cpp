...
public:
	...
	virtual void BeginState_Internal(T prevState) override {
		...
		kbString cur, next;
		if (this->m_pActorComponent->GetOwner()->GetComponent<kbSkeletalModelComponent>()->GetCurAnimationName() != nullptr) {
			cur = *this->m_pActorComponent->GetOwner()->GetComponent<kbSkeletalModelComponent>()->GetCurAnimationName();
		}

		if (this->m_pActorComponent->GetOwner()->GetComponent<kbSkeletalModelComponent>()->GetNextAnimationName() != nullptr) {
			next = *this->m_pActorComponent->GetOwner()->GetComponent<kbSkeletalModelComponent>()->GetNextAnimationName();
		}
