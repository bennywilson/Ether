...
struct worldObjectInfo_t {
	kbVec3 loc;
	int type;
	const kbDQuadEnviroObject * m_pEnviroObject;
};

void GenerateFlatShadedTriangleIndices( vertexLayout *const pInputVerts, vertexLayout *const pOutputVerts, unsigned long *const m_IndexBufferOutput, kbVec3 * collisionOutput, int cellDimensions, kbTerrainCellGenJob *const pTerrainJob, kbVec3 * dynamicCollisionOuput ) {
	...
	for ( int z = 0; z < cellDimensions; z++ ) {
		for ( int x = 0; x < cellDimensions; x++ ) {
			...
			// Collision triangles
			collisionOutput[iVertexBuffer + 0] = pOutVerts[iVertexBuffer + 0].position;
			collisionOutput[iVertexBuffer + 1] = pOutVerts[iVertexBuffer + 1].position;
			collisionOutput[iVertexBuffer + 2] = pOutVerts[iVertexBuffer + 2].position;
			collisionOutput[iVertexBuffer + 3] = pOutVerts[iVertexBuffer + 3].position;
			collisionOutput[iVertexBuffer + 4] = pOutVerts[iVertexBuffer + 4].position;
			collisionOutput[iVertexBuffer + 5] = pOutVerts[iVertexBuffer + 5].position;

			for ( int iPass = 0; iPass < 2; iPass++ ) {
				float RandChance = 0.5f;
				const std::vector<kbDQuadEnviroObject> * pEnviroObjectList = NULL;
				...
				if ( pEnviroObjectList == NULL ) {
					continue;
				}

				float intPart;
				float RandGen = NormalizedNoise( pInputVerts[order[4]].position.x * 0.000100f, pInputVerts[order[4]].position.z * 0.000100f);
				RandGen = modf( RandGen * 100.0f, &intPart );
				if ( RandGen < RandChance ) {
					continue;
				}

				worldObjectInfo_t newWorldObjectInfo;
				newWorldObjectInfo.loc = pInputVerts[order[4]].position;
	...
	for ( int iObj = 0; iObj < worldObjects.size(); iObj++ ) {
		...
		const kbDQuadEnviroObject & EnviroObject = *worldObjects[iObj].m_pEnviroObject;
		...
		pTerrainJob->m_NumDynamicObjects++;
		...
		for ( int iIndex = 0; iIndex < dynIdxSize; iIndex++, iIndexBuffer++, iCollisionBuffer++ ) {
			...
			dynamicCollisionOuput[iCollisionBuffer] = newVert;
		}
	}
...
/**
 *	kbTerrainCellGenJob::kbTerrainCellGenJob
 */
kbTerrainCellGenJob::kbTerrainCellGenJob() :
	m_NumTerrainChunks( 0 ),
	m_ChunkDimensions( 0 ),
	m_ChunkWorldLength( 0 ),
	m_TerrainGenerationNoiseScale( 0.0f ),
	m_MaxTerrainHeight( 0.0f ),
	m_MaxTerrainCellMidPointHeight( 0.0f ),
	m_EnviroComponent( NULL ),
	m_VertexBufferOutput( NULL ),
	m_IndexBufferOutput( NULL ),
	m_CollisionMeshOutput( NULL ),
	m_DynamicCollisionMeshOutput( NULL ),
...
/**
 *	kbTerrainCellGenJob::Run
 */
void kbTerrainCellGenJob::Run() {

	const float ChunkHalfLength = m_ChunkWorldLength * 0.5f;
	const float QuadLength = (float)( m_ChunkWorldLength / m_ChunkDimensions );
	const int numVertsPerSide = m_ChunkDimensions + 1;
	const int totalNumVerts = numVertsPerSide * numVertsPerSide;
...
#define CalculateVertHeightForIndex(vertIndex) pTempTerrainVerts[vertIndex].position.y =  NormalizedNoise( (float) pTempTerrainVerts[vertIndex].position.x * m_TerrainGenerationNoiseScale, ( float ) pTempTerrainVerts[vertIndex].position.z * m_TerrainGenerationNoiseScale ) * m_MaxTerrainHeight;
	...
	m_NumDynamicVertices = 0;
	m_NumDynamicIndices = 0;
	GenerateFlatShadedTriangleIndices( pTempTerrainVerts, m_VertexBufferOutput, m_IndexBufferOutput, m_CollisionMeshOutput, m_ChunkDimensions, this, m_DynamicCollisionMeshOutput );
